...
function tabsFor(isFaculty: boolean): { key: TabKey; label: string; href: '/' | SecondTabHref | '/academics' | '/my-bus'; Icon: typeof HomeIcon }[] {
  return [
...
function activeTabFor(pathname: string): TabKey {
  ...
  if (pathname.startsWith('/my-class') || pathname.startsWith('/fees') || pathname.startsWith('/erp') || pathname.startsWith('/events') || pathname.startsWith('/permissions')) return 'school';
  ...
  return (
    <View style={[styles.bar, { paddingBottom: Math.max(insets.bottom, 10) }]}>
      {TABS.map((tab) => {
        ...
        return (
          <Pressable key={tab.key} onPress={() => router.replace(tab.href)} style={styles.tab} hitSlop={4}>
