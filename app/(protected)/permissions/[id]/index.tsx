...
export default function PermissionRequestDetailScreen() {
  ...
  const detailQuery = useQuery({
    ...
    queryFn: () => getPermissionRequest(id!),
    enabled: !!id,
  });
  ...
  function confirmReject() {
    Alert.alert(
      'Reject this request?',
      'This cannot be undone from here — the teacher will need to add the student again to re-request.',
      [
  ...
  return (
    <View style={styles.flex}>
      <AppHeader
        title={detail?.event.name ?? 'Permission request'}
        onBack={() => router.replace('/permissions' as never)}
      />
          ...
          <>
            <View style={[styles.card, cardShadow]}>
              ...
              <Text style={styles.rowValue}>
                {formatDate(detail.event.startsAt)} · {formatTime(detail.event.startsAt)} –{' '}
                {formatTime(detail.event.endsAt)}
              </Text>
            ...
            {detail.participant.state === 'PENDING' ? (
              <View style={styles.actionsRow}>
                ...
                <Pressable style={styles.signButton} onPress={() => router.push(`/permissions/${id}/sign` as never)}>
              ...
              <Pressable style={styles.letterButton} onPress={handleDownloadLetter} disabled={downloadingLetter}>
                {downloadingLetter ? (
                  ...
                  <Text style={styles.letterButtonText}>Download permission letter</Text>
                )}
              </Pressable>
            ) : (
              <View style={styles.rejectedBanner}>
                <Text style={styles.rejectedBannerText}>
                  ...
                  {detail.participant.decidedAt ? ` on ${formatDate(detail.participant.decidedAt)}` : ''}.
                </Text>
              </View>
            )}
          </>
