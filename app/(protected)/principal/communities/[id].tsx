// Principal -> Community detail -- view-only, real backend
...
// lookup, same cross-module reuse pattern as every prior phase.
...
} from '@/lib/principal-communities-api';
...
export default function PrincipalCommunityDetail() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();
  const [tab, setTab] = useState<Tab>('activities');

  const communityQuery = useQuery({ queryKey: ['principal-communities', 'detail', id], queryFn: () => getCommunity(id) });
  const inchargeId = communityQuery.data?.inchargeStaffId ?? null;
  const inchargeQuery = useQuery({
    queryKey: ['principal-communities', 'incharge', inchargeId],
...
function ActivitiesTab({ communityId }: { communityId: string }) {
  const activitiesQuery = useQuery({
    queryKey: ['principal-communities', 'activities', communityId],
...
function MembersTab({ communityId }: { communityId: string }) {
  const membershipsQuery = useQuery({
    queryKey: ['principal-communities', 'memberships', communityId],
...
function AnnouncementsTab({ communityId }: { communityId: string }) {
  const announcementsQuery = useQuery({
    queryKey: ['principal-communities', 'announcements', communityId],
