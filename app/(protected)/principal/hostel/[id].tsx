// Principal -> Hostel detail -- view-only, real backend data
...
// name resolved via getFaculty (identical class-level access for PRINCIPAL as
// VICE_PRINCIPAL, confirmed by direct backend audit) -- no new lookup
// endpoint. Room/bed capacity computed via a real structural walk
...
export default function PrincipalHostelDetail() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();

  const hostelQuery = useQuery({ queryKey: ['principal-hostel', 'hostel', id], queryFn: () => getHostel(id) });
  const structureQuery = useQuery({ queryKey: ['principal-hostel', 'structure', id], queryFn: () => getHostelStructure(id) });
  const allocationsQuery = useQuery({
    queryKey: ['principal-hostel', 'allocations', 'active'],
    queryFn: () => listAllocations({ status: 'ACTIVE' }),
  });
  const wardenQuery = useQuery({
    queryKey: ['principal-hostel', 'warden', hostelQuery.data?.wardenStaffId],
