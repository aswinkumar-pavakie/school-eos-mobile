// Principal -> Appraisal -- real backend now (faculty-appraisal.controller.ts
...
// principal-appraisal-api.ts's own comment). Adapted from the Faculty
// module's own real screen (app/(protected)/faculty/appraisal.tsx). The
// STAFF_APPRAISAL policy names PRINCIPAL as its own single approver step --
// for a Principal's own appraisal this means self-review, the same disclosed
// shape already accepted for STAFF_LEAVE_REQUEST elsewhere in this app, so
// the subtitle says so plainly rather than the Faculty screen's "Reviewed by
// the Principal" (which would be misleading here).
...
import { listAppraisals, createAppraisal, type StaffAppraisal } from '@/lib/principal-appraisal-api';
import { formatDate } from '@/lib/format';
import { principalColors } from '@/lib/theme';

export default function PrincipalAppraisalScreen() {
  ...
  const listQuery = useQuery({ queryKey: ['principal-appraisal'], queryFn: listAppraisals });
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['principal-appraisal'] });
  ...
  return (
    <View style={styles.flex}>
      <PrincipalHeader title="Appraisal" subtitle="Self-assessment cycles" onBack={() => router.back()} />
      <ScrollView
            ...
            <ScrollView keyboardShouldPersistTaps="handled">
              <View style={styles.sheetHeader}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.sheetSubtitle}>Self-reviewed (Principal is the sole approver on this policy)</Text>
                </View>
  ...
  return (
    <View style={styles.card}>
      <View style={styles.cardTop}>
        ...
        <View style={[styles.stateBadge, reviewed && { backgroundColor: principalColors.greenBg }]}>
          <Text style={[styles.stateBadgeText, reviewed && { color: principalColors.green }]}>{reviewed ? 'Reviewed' : 'Submitted'}</Text>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: principalColors.surface, borderWidth: 1, borderColor: principalColors.border, borderRadius: 16, padding: 15 },
  cardTop: { flexDirection: 'row', alignItems: 'flex-start', gap: 10 },
  cardTitle: { fontSize: 14.5, fontFamily: 'PlusJakartaSans_800ExtraBold', color: principalColors.ink },
  cardMeta: { fontSize: 11.5, fontFamily: 'PlusJakartaSans_600SemiBold', color: principalColors.muted, marginTop: 3 },
  cardBody: { fontSize: 13, color: principalColors.body, lineHeight: 19, marginTop: 9, fontFamily: 'PlusJakartaSans_500Medium' },
  ...
  reviewBox: { marginTop: 10, borderTopWidth: 1, borderTopColor: principalColors.borderSoft, paddingTop: 9, gap: 4 },
  reviewLine: { fontSize: 12.5, fontFamily: 'PlusJakartaSans_600SemiBold', color: principalColors.body },
