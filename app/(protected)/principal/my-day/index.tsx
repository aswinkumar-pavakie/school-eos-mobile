// Principal -> My Day / Personal Overview -- a pure aggregation/presentation
// screen over five ALREADY-REAL, already-authorized endpoints; no new
// backend route, no new table, no new approval/attendance/leave/notification
// engine. Every section below reuses the exact same self-scoped API this
// Principal module already calls from its own dedicated screen:
//   - Today's attendance + attendance summary -> getMyAttendanceHistory()
//     (GET /staff/me/attendance-history, staffId resolved server-side from
//     the caller's own personId -- PRINCIPAL already authorized identical to
//     VICE_PRINCIPAL, see principal-my-attendance-api.ts's own comment)
//   - Personal requests -> listMyLeaveRequests() (GET /staff/me/leave-requests,
//     same self-scoping and same evidence, see principal-my-leave-api.ts's
//     own comment for the disclosed single-reviewer-role caveat)
//   - Pending actions awaiting Principal's decision -> listApprovals({status:
//     'PENDING'}) (GET /approvals, reviewer-scoped server-side by
//     listForCaller() against the caller's own role_assignment rows) --
//     UNLIKE Vice Principal's own My Day (whose equivalent list is real but
//     always empty, no approval_policy names VICE_PRINCIPAL), Principal is a
//     genuinely active approver today (REFUND/FEE_CONCESSION/FEE_STRUCTURE/
//     EXPENSE_ABOVE_PETTY plus community/faculty/media workflows), so this
//     section shows real, live requests, not a permanently-empty state.
//   - Upcoming items -> listUpcomingCalendarEvents() (GET /calendar-events,
//     re-exported for this screen only -- see principal-dashboard-api.ts's
//     own comment on why it's not part of Principal's own real web dashboard
//     but is still real, already-authorized data for this personal overview)
//   - Notifications -> listNotifications() (GET /notifications, self-scoped
//     to the caller's own personId, no @Roles restriction)
//
// "My Responsibilities" is deliberately NOT rendered -- there is no such
// module, table, or endpoint anywhere in this codebase (verified by search).
// Faking a section for it would violate this app's own "no invented KPIs, no
// mock data" rule, so it is omitted entirely.
//
...
import { listApprovals } from '@/lib/principal-requests-approvals-api';
...
export default function PrincipalMyDayScreen() {
  const router = useRouter();

  const attendanceQuery = useQuery({ queryKey: ['principal-my-day', 'attendance'], queryFn: () => getMyAttendanceHistory() });
  const leaveQuery = useQuery({ queryKey: ['principal-my-day', 'my-leave'], queryFn: listMyLeaveRequests });
  const pendingActionsQuery = useQuery({
    queryKey: ['principal-my-day', 'pending-actions'],
    queryFn: () => listApprovals({ status: 'PENDING' }),
  });
  const eventsQuery = useQuery({ queryKey: ['principal-my-day', 'calendar-events'], queryFn: listUpcomingCalendarEvents });
  const notificationsQuery = useQuery({
    queryKey: ['principal-my-day', 'notifications'],
    ...
    <View style={styles.flex}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.sectionHeaderRow}>
          ...
          <Pressable onPress={() => router.push('/(protected)/principal/my-leave' as never)}>
        ...
        <View style={styles.sectionHeaderRow}>
          ...
          <Pressable onPress={() => router.push('/(protected)/principal/requests-approvals' as never)}>
            <Text style={styles.viewAll}>View all</Text>
          </Pressable>
        </View>
        ...
        <View style={styles.sectionHeaderRow}>
          ...
          <Pressable onPress={() => router.push('/(protected)/principal/notifications' as never)}>
            <Text style={styles.viewAll}>View all</Text>
          </Pressable>
        </View>
        ...
        <View style={styles.actionsWrap}>
          {QUICK_ACTIONS.map((action) => (
            <Pressable
              ...
              onPress={() => router.push(`/(protected)/principal/${action.slug}` as never)}
            >
