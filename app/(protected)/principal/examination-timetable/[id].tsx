// Principal -> Examination Timetable detail -- read-only, real backend data
// only. No create/edit/publish/room-assignment actions -- exams.controller.ts
// only grants PRINCIPAL the read methods this screen calls; every write
// method stays ADMIN-only, enforced server-side. Mirrors Vice Principal's
// own detail screen exactly. Schedule grouped by real exam date, sorted by
// date then start time -- deliberately shows scheduling fields only
// (date/time/room/subject/class/teacher), never the marks/practical fields
// the same rows also carry (Examinations module, out of this screen's
// scope).
...
import { getExamSchedules, getExamination, type ExamScheduleRow } from '@/lib/principal-exam-timetable-api';
...
export default function PrincipalExamTimetableDetail() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();

  const examQuery = useQuery({ queryKey: ['principal-exam-timetable', 'exam', id], queryFn: () => getExamination(id) });
  const schedulesQuery = useQuery({ queryKey: ['principal-exam-timetable', 'schedules', id], queryFn: () => getExamSchedules(id) });

  if (examQuery.isLoading) {
    return (
      <View style={styles.flex}>
        <PrincipalHeader title="Examination" onBack={() => router.back()} />
  ...
  return (
    <View style={styles.flex}>
      <PrincipalHeader title={exam.name} subtitle={exam.academicYearName} onBack={() => router.back()} />
...
const styles = StyleSheet.create({
  ...
  dateHeading: { fontSize: 13.5, fontFamily: 'PlusJakartaSans_800ExtraBold', color: principalColors.muted, marginBottom: 8 },
  ...
  timeCol: { width: 66 },
  timeText: { fontSize: 13, fontFamily: 'PlusJakartaSans_800ExtraBold', color: principalColors.ink },
  durationText: { fontSize: 11, fontFamily: 'PlusJakartaSans_600SemiBold', color: principalColors.muted, marginTop: 2 },
