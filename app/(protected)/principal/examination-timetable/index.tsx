// Principal -> Examination Timetable -- full feature parity with the
// website's own /principal/academics/examination-timetable page, previously
// mobile-missing. List of real examinations (reuses listExaminations rather
// than duplicating it), tap through to each one's real per-subject
// schedule. Mirrors Vice Principal's own already-built mobile module
// exactly (same real endpoints, PRINCIPAL already granted equally).
...
import { listExaminations } from '@/lib/principal-exam-timetable-api';
...
export default function PrincipalExamTimetableScreen() {
  const router = useRouter();
  const examsQuery = useQuery({ queryKey: ['principal-exam-timetable', 'exams'], queryFn: () => listExaminations() });
  const exams = examsQuery.data ?? [];

  return (
    <View style={styles.flex}>
      <PrincipalHeader title="Examination Timetable" subtitle="Scheduled examinations" onBack={() => router.back()} />
            ...
            {exams.map((exam, index) => {
              ...
              return (
                <Pressable
                  ...
                  onPress={() => router.push(`/(protected)/principal/examination-timetable/${exam.id}` as never)}
                >
