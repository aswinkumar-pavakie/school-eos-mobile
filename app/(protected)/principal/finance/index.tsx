// Principal -> Finance -- school-level financial oversight, real backend data
// only (fee-overview/fee-demands/fee-heads/fee-structures/payments -- the
// SAME endpoints VP already uses, confirmed identical @Roles by direct
// backend audit). Adds a fourth tab VP does NOT have: Purchase Requests --
// finance/purchase-requests.controller.ts grants POST specifically to
// @Roles('PRINCIPAL') alone (not even FINANCE/ADMIN), a genuine
// Principal-exclusive capability with a real, correctly-typed web API client
// function nothing ever called -- adapting it here, not inventing it.
// Concessions/Expenses stay drill-through-only from a Requests & Approvals
// decision (matching Principal's own real web app), not a standalone tab.
...
import {
  ...
  listPayments,
} from '@/lib/principal-finance-api';
...
type Tab = 'outstanding' | 'structures' | 'payments' | 'purchase-requests';
...
function purchaseRequestStateTone(state: string): StatusTone {
  if (state === 'APPROVED' || state === 'COMPLETED') return 'positive';
  if (state === 'REJECTED' || state === 'CANCELLED') return 'negative';
  return 'warning';
}

export default function PrincipalFinanceScreen() {
  const router = useRouter();
  const [tab, setTab] = useState<Tab>('outstanding');
  const [academicYearId, setAcademicYearId] = useState<string | null>(null);

  const yearsQuery = useQuery({ queryKey: ['principal-finance', 'years'], queryFn: listAcademicYears });
  const gradesQuery = useQuery({ queryKey: ['principal-finance', 'grades'], queryFn: listGrades });
  ...
  const overviewQuery = useQuery({
    queryKey: ['principal-finance', 'overview', effectiveYearId],
    ...
    <View style={styles.flex}>
      ...
      <SegmentedTabs<Tab>
        tabs={[
          ...
          { key: 'purchase-requests', label: 'My Requests' },
        ]}
        value={tab}
        onChange={setTab}
      />
      ...
      {tab === 'outstanding' ? (
        ...
        <PurchaseRequestsTab router={router} />
      )}
    </View>
  );
}
...
function OutstandingFeesTab({ academicYearId, grades }: { academicYearId?: string; grades: { id: string; name: string }[] }) {
  ...
  const demandsQuery = useQuery({
    queryKey: ['principal-finance', 'demands', academicYearId, search, state, gradeId],
    queryFn: () =>
      listFeeDemands({
        academicYearId,
...
function FeeStructuresTab({
  ...
  const feeHeadsQuery = useQuery({ queryKey: ['principal-finance', 'fee-heads'], queryFn: listFeeHeads });
  const structuresQuery = useQuery({
    queryKey: ['principal-finance', 'structures', academicYearId, gradeId, state],
        ...
        <View style={styles.list}>
          {structures.map((structure, index) => (
            <Pressable
              ...
              onPress={() => router.push(`/(protected)/principal/finance/structures/${structure.id}` as never)}
            >
              <View style={{ flex: 1, minWidth: 0 }}>
                <Text style={styles.rowTitle} numberOfLines={1}>
...
function PaymentsTab({ academicYearId }: { academicYearId?: string }) {
  ...
  const paymentsQuery = useQuery({
    queryKey: ['principal-finance', 'payments', academicYearId, search, state],
...
function PurchaseRequestsTab({ router }: { router: ReturnType<typeof useRouter> }) {
  const listQuery = useQuery({ queryKey: ['principal-finance', 'purchase-requests'], queryFn: () => listMyPurchaseRequests() });
  const requests = listQuery.data?.data ?? [];

  return (
    <ScrollView contentContainerStyle={styles.tabContent}>
      <Pressable style={styles.applyButton} onPress={() => router.push('/(protected)/principal/finance/purchase-requests/apply' as never)}>
        <Text style={styles.applyButtonText}>+ Raise a purchase / service request</Text>
      </Pressable>
      ...
      {listQuery.isLoading ? (
        ...
        <EmptyState message="You haven't raised any purchase or service requests yet." />
      ) : (
        <View style={styles.list}>
          {requests.map((r, index) => (
            <View key={r.id} style={[styles.row, index === 0 && styles.rowFirst]}>
              <View style={{ flex: 1, minWidth: 0 }}>
                <Text style={styles.rowTitle} numberOfLines={1}>
                  {r.itemName}
                </Text>
                <Text style={styles.rowMeta} numberOfLines={1}>
                  {humanize(r.requestType)} · {formatDate(r.createdAt)}
                </Text>
              </View>
              <StatusBadge label={humanize(r.state)} tone={purchaseRequestStateTone(r.state)} />
            </View>
          ))}
        </View>
      )}
