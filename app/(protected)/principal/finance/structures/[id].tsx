// Principal -> Finance -> Fee Structure detail -- view-only, real backend
// data only. No create/update/publish/supersede/line actions -- the
// read-only /fee-structures controller VP already uses is the one this
// screen calls; the write-capable finance/fee-structures/ controller is a
// completely separate FINANCE/ADMIN-only controller, never reached here.
...
export default function PrincipalFeeStructureDetail() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();

  const structureQuery = useQuery({ queryKey: ['principal-finance', 'structure', id], queryFn: () => getFeeStructure(id) });
