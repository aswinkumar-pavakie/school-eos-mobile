// Principal -> Notifications -- real backend data only, via GET
// /notifications and POST /notifications/:id/read (no @Roles restriction at
// all -- self-scoped to notification.person_id, open to any authenticated
// actor, confirmed by direct backend audit). These are Principal's own,
// personal notifications -- never another person's, another school's, or a
// role-wide feed; there is no such thing as a "role-wide" notification in
// the real system, every row is already targeted at one specific person by
// whatever module raised it. There is no dedicated Principal web
// notification inbox (confirmed by audit -- web only shows a bare
// pending-approvals count badge), but this backend capability genuinely
// already exists and is real, so this adapts an authorized capability
// rather than inventing one.
//
// Deep-linking: only a notification whose relatedObjectType is
// 'approval_request' gets a "View request" action, into Principal's own
// already-authorized Requests & Approvals detail screen -- that screen
// performs its own real GET /approvals/:id authorization check
// independently, so tapping through here can never bypass it.
...
export default function PrincipalNotificationsScreen() {
  ...
  const listQuery = useQuery({
    queryKey: ['principal-notifications', 'list', tab],
  ...
  async function handlePress(notification: NotificationRow) {
    ...
    if (!notification.readAt) {
      try {
        ...
        queryClient.invalidateQueries({ queryKey: ['principal-notifications'] });
      } catch {
                ...
                >
                  ...
                  {expanded && notification.relatedObjectType === 'approval_request' && notification.relatedObjectId ? (
                    <Pressable
                      ...
                      onPress={() =>
                        router.push(`/(protected)/principal/requests-approvals/${notification.relatedObjectId}` as never)
                      }
                    >
