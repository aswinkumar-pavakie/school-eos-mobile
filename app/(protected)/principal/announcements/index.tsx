// Principal -> Announcements -- same real GET /announcements list VP already
// uses. Adds a "+ New" button VP does NOT have: announcements.controller.ts's
// class-level @Roles('ADMIN', 'PRINCIPAL') already covers POST /announcements
// with no method-level override -- Principal has full create parity with
// Admin, confirmed by direct backend audit. archive() stays ADMIN-only, not
// built here. Guarded by the parent principal/_layout.tsx.
...
export default function PrincipalAnnouncementsScreen() {
  ...
  const announcementsQuery = useQuery({
    queryKey: ['principal-announcements', 'list', audienceRole, includeArchived],
            ...
            {filtered.map((announcement, index) => {
              ...
              return (
                <Pressable
                  ...
                  onPress={() => router.push(`/(protected)/principal/announcements/${announcement.id}` as never)}
                >
