// Principal -> Academic Calendar event detail -- view-only, real backend
// data only. No create/edit/delete actions -- writes stay ADMIN-only on the
// real serving controller (academic/calendar-events.controller.ts), enforced
// server-side. Scope resolved via the same /grades /sections calls -- no new
// lookup endpoint.
...
export default function PrincipalAcademicCalendarDetail() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();

  const eventQuery = useQuery({ queryKey: ['principal-calendar', 'event', id], queryFn: () => getCalendarEvent(id) });
  const gradesQuery = useQuery({
    queryKey: ['principal-calendar', 'grades'],
    queryFn: listGrades,
    enabled: eventQuery.data?.scopeType === 'GRADE' || eventQuery.data?.scopeType === 'SECTION',
  });
  const sectionsQuery = useQuery({
    queryKey: ['principal-calendar', 'sections'],
