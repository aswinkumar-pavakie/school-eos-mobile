// Principal -> Parent detail -- view-only, real backend data only. No
// relationship modification -- that stays ADMIN-only on
// guardian-links.controller.ts, enforced server-side. Tapping a linked
// student navigates into the real Principal Student detail screen
// (principal/students/[id].tsx) -- no student data duplicated here.
...
export default function PrincipalParentDetail() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();

  const parentQuery = useQuery({ queryKey: ['principal-parents', 'detail', id], queryFn: () => getParent(id) });
            ...
            {parent.children.map((child, index) => {
              ...
              return (
                <Pressable
                  ...
                  onPress={() => router.push(`/(protected)/principal/students/${child.studentId}` as never)}
                >
