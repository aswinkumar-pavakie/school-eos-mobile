// Principal -> Leave request detail -- view-only plus Withdraw, real backend
// data only. Withdraw reuses the generic approvals engine's own existing
// POST /approvals/:id/withdraw (requester-only, still-open-only, already
// enforced there) -- not a new cancel workflow. No edit action -- there is
// no update endpoint for a submitted leave request in the real backend.
//
// Note: the "Awaiting decision" step shown below for a still-pending request
// may never resolve to a decision by design -- see principal-my-leave-api.ts's
// own comment on the STAFF_LEAVE_REQUEST approval_policy's single PRINCIPAL
// reviewer role and the self-approval block. Withdraw remains the Principal's
// real, working way to close out such a request.
...
import { getMyLeaveRequest, withdrawMyLeaveRequest } from '@/lib/principal-my-leave-api';
...
export default function PrincipalMyLeaveDetail() {
  const router = useRouter();
  const queryClient = useQueryClient();
  const { id } = useLocalSearchParams<{ id: string }>();
  const [withdrawing, setWithdrawing] = useState(false);

  const query = useQuery({ queryKey: ['principal-my-leave', 'detail', id], queryFn: () => getMyLeaveRequest(id) });
  ...
  if (query.isLoading) {
    return (
      <View style={styles.flex}>
        <PrincipalHeader title="Leave request" onBack={() => router.back()} />
  ...
  return (
    <View style={styles.flex}>
      <PrincipalHeader title={humanize(leave.leaveType)} onBack={() => router.back()} />
