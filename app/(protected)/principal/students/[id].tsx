...
export default function PrincipalStudentDetail() {
  ...
  if (studentQuery.isLoading) {
    return (
      <View style={styles.flex}>
        <PrincipalHeader title="Student" onBack={() => router.back()} />
  ...
  return (
    <View style={styles.flex}>
      <PrincipalHeader title={`${student.firstName} ${student.lastName ?? ''}`} subtitle={student.admissionNo} onBack={() => router.back()} />
...
const styles = StyleSheet.create({
  ...
  attendanceValue: { fontSize: 20, fontFamily: 'PlusJakartaSans_800ExtraBold', color: principalColors.ink },
});
