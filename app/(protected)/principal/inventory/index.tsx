// Principal -> Inventory -- school-level operational
...
// -- see principal-inventory-api.ts's own comment). Guarded by the parent
...
import { getInventoryOverview, listInventoryCategories, listInventoryItems } from '@/lib/principal-inventory-api';
...
export default function PrincipalInventoryScreen() {
  ...
  const overviewQuery = useQuery({ queryKey: ['principal-inventory', 'overview'], queryFn: getInventoryOverview });
  const categoriesQuery = useQuery({ queryKey: ['principal-inventory', 'categories'], queryFn: listInventoryCategories });
  const category = categoriesQuery.data?.find((c) => c.name === categoryName) ?? null;

  const itemsQuery = useQuery({
    queryKey: ['principal-inventory', 'items', search, location, status, category?.id],
    queryFn: () =>
      listInventoryItems({
  ...
  return (
    <View style={styles.flex}>
      <PrincipalHeader title="Inventory" subtitle="School-wide inventory overview" onBack={() => router.back()} />
            ...
            <View style={styles.list}>
              {items.map((item, index) => (
                <Pressable
                  ...
                  onPress={() => router.push(`/(protected)/principal/inventory/${item.id}` as never)}
                >
