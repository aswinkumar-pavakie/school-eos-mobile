// Principal -> Attendance Sessions -- class-level attendance oversight, a
// REAL, distinct module from the existing /principal/attendance screen
// (staff daily roll call). Mirrors Vice Principal's own already-built
// mobile module exactly (same real endpoints, now also granted to
// PRINCIPAL) -- full feature parity with the website's own
// /principal/attendance-sessions page, previously mobile-missing.
// Read-only: list/get only, matching the real backend surface -- no
// create/lock controls, those stay ADMIN-only.
...
import { getSession, listSessions, type AttendanceStatus } from '@/lib/principal-attendance-sessions-api';
...
export default function PrincipalAttendanceSessionsScreen() {
  const router = useRouter();
  const [date, setDate] = useState(todayIso());
  const [gradeName, setGradeName] = useState<string | null>(null);
  const [sectionName, setSectionName] = useState<string | null>(null);

  const overviewQuery = useQuery({ queryKey: ['principal-attendance-sessions', 'overview', date], queryFn: () => computeOverview(date) });

  const gradesQuery = useQuery({ queryKey: ['principal-attendance-sessions', 'grades'], queryFn: listGrades });
  const grade = useMemo(() => gradesQuery.data?.find((g) => g.name === gradeName) ?? null, [gradesQuery.data, gradeName]);
  const sectionsQuery = useQuery({
    queryKey: ['principal-attendance-sessions', 'sections', grade?.id],
  ...
  const sessionsQuery = useQuery({
    queryKey: ['principal-attendance-sessions', 'sessions', date, section?.id],
  ...
  return (
    <View style={styles.flex}>
      <PrincipalHeader title="Attendance Sessions" subtitle="Class-level oversight" onBack={() => router.back()} />
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {overviewQuery.isLoading ? (
          <ActivityIndicator color={principalColors.primary} style={{ marginVertical: 16 }} />
          ...
          <View style={styles.list}>
            {sessions.map((session, index) => (
              <Pressable
                ...
                onPress={() => router.push(`/(protected)/principal/attendance-sessions/${session.id}` as never)}
              >
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 14, fontFamily: 'PlusJakartaSans_800ExtraBold', color: principalColors.ink, marginTop: 10, marginBottom: 10 },
  ...
  breakdownLabel: { fontSize: 10.5, fontFamily: 'PlusJakartaSans_600SemiBold', color: principalColors.muted, textAlign: 'center' },
