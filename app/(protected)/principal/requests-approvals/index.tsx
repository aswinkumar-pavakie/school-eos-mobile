// Principal -> Requests & Approvals -- real backend data only, via the same
// generic approvals engine every other approval-routed feature in this app
// already uses (see principal-requests-approvals-api.ts's own comment for
// the full authorization model). Read-only list; decisions happen on the
// detail screen. Guarded by the parent principal/_layout.tsx.
//
// UNLIKE Vice Principal (whose list is real but always empty -- no
// approval_policy names VICE_PRINCIPAL): Principal genuinely is an active
// approver today, confirmed by direct backend audit of real approval_policy
// rows (REFUND, FEE_CONCESSION, FEE_STRUCTURE, EXPENSE_ABOVE_PETTY, plus
// community/faculty/media workflows). This list will show real requests
// requiring a decision, not just an honest empty state.
...
export default function PrincipalRequestsApprovalsScreen() {
  const router = useRouter();
  const [tab, setTab] = useState<Tab>('PENDING');

  const listQuery = useQuery({
    queryKey: ['principal-requests-approvals', 'list', tab],
  ...
  return (
    <View style={styles.flex}>
      <PrincipalHeader title="Requests & Approvals" subtitle="Requests awaiting your review" onBack={() => router.back()} />
