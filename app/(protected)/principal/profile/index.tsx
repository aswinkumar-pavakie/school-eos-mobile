// Principal -> Profile -- read-only, real backend data only. Name/email/roles
// from the existing, shared useMe() hook; designation/employee no/photo/join
// date from the self-scoped GET /staff/me (already grants PRINCIPAL,
// identical to VICE_PRINCIPAL's own grant); school name/board/address/
// contact from GET /school (also already broadened to PRINCIPAL). No edit
// action anywhere -- no self-service write path exists in the real backend
// for any role. There is no dedicated Principal web Profile page (confirmed
// by audit -- the shared web Shell has no profile concept at all), but both
// backend capabilities genuinely already exist and are real, so this adapts
// an authorized capability rather than inventing one. Guarded by the parent
...
export default function PrincipalProfileScreen() {
  ...
  const meQuery = useMe();
  const staffQuery = useQuery({ queryKey: ['principal-profile', 'staff'], queryFn: getMyStaffProfile });
  const schoolQuery = useQuery({ queryKey: ['principal-profile', 'school'], queryFn: getSchoolInfo });
