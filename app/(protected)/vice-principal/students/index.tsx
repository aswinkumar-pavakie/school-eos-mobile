// Vice Principal -> Students (Phase 4) -- an oversight/information list, real
// backend search+filters only (students.controller.ts's own /students, now
// also authorized for VICE_PRINCIPAL -- see that controller's comment).
// Guarded by the parent vice-principal/_layout.tsx (covers this whole
// subtree, no separate role check needed here).
...
import { listGrades, listSections, listStudents } from '@/lib/vice-principal-students-api';
...
export default function VicePrincipalStudentsScreen() {
  ...
  const gradesQuery = useQuery({ queryKey: ['vp-students', 'grades'], queryFn: listGrades });
  const grade = useMemo(() => gradesQuery.data?.find((g) => g.name === gradeName) ?? null, [gradesQuery.data, gradeName]);

  const sectionsQuery = useQuery({
    queryKey: ['vp-students', 'sections', grade?.id],
  ...
  const listQuery = useQuery({
    queryKey: ['vp-students', 'list', search, status, grade?.id, section?.id],
    queryFn: () =>
      listStudents({
              ...
              {students.map((student, index) => {
                ...
                return (
                  <Pressable
                    ...
                    onPress={() => router.push(`/(protected)/vice-principal/students/${student.id}` as never)}
                  >
                    {student.photoUrl ? (
