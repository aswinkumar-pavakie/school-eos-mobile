// Vice Principal -> Vehicle detail (Phase 13) -- view-only, real backend
// data only. No edit/delete/document/maintenance actions -- vehicles.
// controller.ts only grants VICE_PRINCIPAL list/get; every write method, plus
// the documents/maintenance sub-resources, stay ADMIN+PRINCIPAL only,
// enforced server-side (maintenance is explicitly out of this phase's scope
// -- see the api file's own comment).
...
import { getVehicle, listAssignments, listDrivers, listRoutes } from '@/lib/vice-principal-transport-api';
...
export default function VicePrincipalVehicleDetail() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();

  const vehicleQuery = useQuery({ queryKey: ['vp-transport', 'vehicle', id], queryFn: () => getVehicle(id) });
  const assignmentQuery = useQuery({
    queryKey: ['vp-transport', 'vehicle-assignment', id],
