// Vice Principal -> Driver detail (Phase 13) -- view-only, real backend data
// only. No edit/delete actions -- drivers.controller.ts only grants
// VICE_PRINCIPAL list/get; every write method stays ADMIN-only, enforced
// server-side. Drivers remain plain transport master records, never
// application users, matching the backend's own comment.
...
import { getDriver, listAssignments, listRoutes, listVehicles } from '@/lib/vice-principal-transport-api';
...
export default function VicePrincipalDriverDetail() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();

  const driverQuery = useQuery({ queryKey: ['vp-transport', 'driver', id], queryFn: () => getDriver(id) });
  ...
  const assignmentsQuery = useQuery({
    queryKey: ['vp-transport', 'current-assignments'],
