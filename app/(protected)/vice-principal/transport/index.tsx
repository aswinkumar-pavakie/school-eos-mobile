// Vice Principal -> Transport (Phase 13) -- school-level operational
// oversight, real backend data only. Vehicles/Routes/Drivers all newly (and
// minimally) authorized for VICE_PRINCIPAL -- see each controller's own
// comment in vice-principal-transport-api.ts. Guarded by the parent
...
import { listDrivers, listRoutes, listVehicles } from '@/lib/vice-principal-transport-api';
...
export default function VicePrincipalTransportScreen() {
  const router = useRouter();
  const [tab, setTab] = useState<'vehicles' | 'routes' | 'drivers'>('vehicles');

  const vehiclesQuery = useQuery({ queryKey: ['vp-transport', 'vehicles'], queryFn: listVehicles });
  const routesQuery = useQuery({ queryKey: ['vp-transport', 'routes'], queryFn: listRoutes });
  const driversQuery = useQuery({ queryKey: ['vp-transport', 'drivers'], queryFn: listDrivers });
          ...
          {tab === 'vehicles' ? (
            <ListSection
              ...
              renderRow={(vehicle) => ({
                ...
                onPress: () => router.push(`/(protected)/vice-principal/transport/vehicles/${vehicle.id}` as never),
              })}
            />
          ) : tab === 'routes' ? (
            <ListSection
              ...
              renderRow={(route) => ({
                ...
                onPress: () => router.push(`/(protected)/vice-principal/transport/routes/${route.id}` as never),
              })}
            />
          ) : (
            <ListSection
              ...
              renderRow={(driver) => ({
                ...
                onPress: () => router.push(`/(protected)/vice-principal/transport/drivers/${driver.id}` as never),
              })}
            />
          )}
        </View>
