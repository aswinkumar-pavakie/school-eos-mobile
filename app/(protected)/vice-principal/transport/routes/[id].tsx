// Vice Principal -> Route detail (Phase 13) -- view-only, real backend data
// only. No edit/delete/stop-management actions -- routes.controller.ts only
// grants VICE_PRINCIPAL list/get/listStops/listAssignedStudents; every write
...
import {
  ...
  listRouteStops,
  listVehicles,
} from '@/lib/vice-principal-transport-api';
...
export default function VicePrincipalRouteDetail() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();

  const routeQuery = useQuery({ queryKey: ['vp-transport', 'route', id], queryFn: () => getRoute(id) });
  const stopsQuery = useQuery({ queryKey: ['vp-transport', 'route-stops', id], queryFn: () => listRouteStops(id) });
  const studentsQuery = useQuery({
    queryKey: ['vp-transport', 'route-students', id],
    queryFn: () => listRouteAssignedStudents(id),
  });
  const assignmentQuery = useQuery({
    queryKey: ['vp-transport', 'route-assignment', id],
