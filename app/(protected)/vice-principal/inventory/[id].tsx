// Vice Principal -> Inventory item detail (Phase 15) -- view-only, real
...
// grants VICE_PRINCIPAL list/overview/get; every write method stays
// ADMIN-only, enforced server-side. acquisitionCostPaise and vendor are
// deliberately never rendered (procurement/financial data, out of scope).
...
import { getInventoryItem } from '@/lib/vice-principal-inventory-api';
...
export default function VicePrincipalInventoryItemDetail() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();

  const itemQuery = useQuery({ queryKey: ['vp-inventory', 'item', id], queryFn: () => getInventoryItem(id) });
