// Vice Principal -> Class Timetable (Phase 9) -- read-only oversight, real
// backend data only (timetable.controller.ts's own GET /timetable, now also
// authorized for VICE_PRINCIPAL -- see that controller's comment). Grade/
// Section pickers reuse the exact same /grades /sections calls already
// authorized for VP in Phase 4 -- no duplicate API. Guarded by the parent
// vice-principal/_layout.tsx. Day-tab + vertical period list (not a dense
// 6-day grid) -- the standard, touch-friendly mobile timetable pattern;
// every real slot already carries its own period label/time/room, so no
// separate periods-master fetch is needed just to render a day's schedule.
...
import { getSectionTimetable } from '@/lib/vice-principal-timetable-api';
...
function currentWeekday(): number {
  ...
  // (no Sunday classes, confirmed against real data) -- map accordingly,
  // falling back to Monday on a Sunday.
  const jsDay = new Date().getDay();
  return jsDay === 0 ? 1 : jsDay;
}

export default function VicePrincipalClassTimetableScreen() {
  const router = useRouter();
  const [gradeName, setGradeName] = useState<string | null>(null);
  const [sectionName, setSectionName] = useState<string | null>(null);
  const [day, setDay] = useState(currentWeekday());

  const gradesQuery = useQuery({ queryKey: ['vp-timetable', 'grades'], queryFn: listGrades });
  const grade = useMemo(() => gradesQuery.data?.find((g) => g.name === gradeName) ?? null, [gradesQuery.data, gradeName]);

  const sectionsQuery = useQuery({
    queryKey: ['vp-timetable', 'sections', grade?.id],
  ...
  const timetableQuery = useQuery({
    queryKey: ['vp-timetable', 'section-timetable', section?.id],
