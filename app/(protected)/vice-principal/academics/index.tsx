// Vice Principal -> Academics (Phase 8) -- school-level academic-structure
// oversight, real backend data only. All four Academics endpoints
// (academic-years/subjects/mediums/departments) newly authorized for
...
// Phase 4; Faculty reuses Phase 6's own listFaculty to resolve a
// department's Head of Department name -- no duplicate API anywhere.
// Deliberately a single overview screen, not a list->detail architecture --
// this is small, mostly-static reference data (a handful of grades/
// subjects/departments/mediums), not a large record set like
// Students/Parents/Faculty, so a scrollable set of grouped cards is the
// right shape, not invented multi-screen navigation.
...
import {
  ...
  listMediums,
  listSubjects,
} from '@/lib/vice-principal-academics-api';
import { listFaculty } from '@/lib/vice-principal-faculty-api';
...
export default function VicePrincipalAcademicsScreen() {
  const router = useRouter();

  const yearsQuery = useQuery({ queryKey: ['vp-academics', 'years'], queryFn: listAcademicYears });
  const gradesQuery = useQuery({ queryKey: ['vp-academics', 'grades'], queryFn: listGrades });
  const sectionsQuery = useQuery({ queryKey: ['vp-academics', 'sections'], queryFn: () => listSections() });
  const subjectsQuery = useQuery({ queryKey: ['vp-academics', 'subjects'], queryFn: listSubjects });
  const departmentsQuery = useQuery({ queryKey: ['vp-academics', 'departments'], queryFn: listDepartments });
  const mediumsQuery = useQuery({ queryKey: ['vp-academics', 'mediums'], queryFn: listMediums });
  const facultyQuery = useQuery({ queryKey: ['vp-academics', 'faculty'], queryFn: () => listFaculty({ limit: 200 }) });
