// Vice Principal -> Reports (Phase 21) -- institution-wide, read-only
// cross-cutting figures, real backend data only (admin/reports-summary, now
// authorized for VICE_PRINCIPAL with requestsApprovals redacted server-side
// -- see vice-principal-reports-api.ts's own comment). No filters, no
// export, no drill-down -- the real backend endpoint offers none of those,
// same single-aggregate shape as Library/Repair & Maintenance's own overview
// screens. Guarded by the parent vice-principal/_layout.tsx.
...
import { getReportsSummary } from '@/lib/vice-principal-reports-api';
...
function genericStatusTone(status: string): StatusTone {
  if (['ACTIVE', 'AVAILABLE', 'PAID'].includes(status)) return 'positive';
  if (['LOST', 'DAMAGED', 'GROUNDED', 'RETIRED'].includes(status)) return 'negative';
  if (['MAINTENANCE', 'UNDER_REPAIR', 'ISSUED', 'RESERVED'].includes(status)) return 'warning';
  return 'neutral';
}
...
export default function VicePrincipalReportsScreen() {
  const router = useRouter();
  const summaryQuery = useQuery({ queryKey: ['vp-reports', 'summary'], queryFn: getReportsSummary });
