// Vice Principal -> Hostel (Phase 14) -- school-level operational oversight,
// real backend data only. All 5 hostel controllers newly (and minimally)
// authorized for VICE_PRINCIPAL -- see vice-principal-hostel-api.ts's own
// comment. Guarded by the parent vice-principal/_layout.tsx.
...
import { listAllocations, listHostels } from '@/lib/vice-principal-hostel-api';
...
export default function VicePrincipalHostelScreen() {
  const router = useRouter();

  const hostelsQuery = useQuery({ queryKey: ['vp-hostel', 'hostels'], queryFn: listHostels });
            ...
            {hostels.map((hostel, index) => {
              ...
              return (
                <Pressable
                  ...
                  onPress={() => router.push(`/(protected)/vice-principal/hostel/${hostel.id}` as never)}
                >
