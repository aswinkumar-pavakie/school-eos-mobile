// Vice Principal -> Announcement detail (Phase 20) -- view-only, real
// backend data only. No edit/archive/publish actions -- those stay ADMIN
// (and create: ADMIN+PRINCIPAL) only, enforced server-side; VP never gets
// them just by being able to read.
//
...
import { listAnnouncements, type AnnouncementAudience } from '@/lib/vice-principal-announcements-api';
...
export default function VicePrincipalAnnouncementDetail() {
  ...
  if (allQuery.isLoading) {
    return (
      <View style={styles.flex}>
        <AppHeader title="Notice" onBack={() => router.back()} />
