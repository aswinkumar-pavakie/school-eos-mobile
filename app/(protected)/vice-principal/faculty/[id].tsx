// Vice Principal -> Faculty detail (Phase 6) -- view-only, real backend data
// only. No edit/create/exit actions -- staff.controller.ts only grants
// VICE_PRINCIPAL the five read methods this screen calls; every write method
// stays ADMIN-only, enforced server-side. "Assignments" is the DISTINCT set
// of classes/sections/subjects derived from real timetable slots -- shown as
// a plain list, never as a day/period grid (that's the Timetable module,
...
import { getAttendanceSummary, getFaculty, listTimetableSlots } from '@/lib/vice-principal-faculty-api';
...
export default function VicePrincipalFacultyDetail() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();

  const facultyQuery = useQuery({ queryKey: ['vp-faculty', 'detail', id], queryFn: () => getFaculty(id) });
  const attendanceQuery = useQuery({ queryKey: ['vp-faculty', 'attendance', id], queryFn: () => getAttendanceSummary(id) });
  const slotsQuery = useQuery({ queryKey: ['vp-faculty', 'timetable', id], queryFn: () => listTimetableSlots(id) });
