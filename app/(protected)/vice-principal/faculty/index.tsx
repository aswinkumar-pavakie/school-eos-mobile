// Vice Principal -> Faculty (Phase 6) -- an oversight/information list, real
// backend search+filters only (staff.controller.ts's own /staff, now also
// authorized for VICE_PRINCIPAL -- see that controller's comment). Grade/
// Section filters reuse the exact same /grades /sections calls Phase 4's
// Students module already authorized for VP -- no duplicate API. Guarded by
// the parent vice-principal/_layout.tsx (covers this whole subtree).
...
import { listDesignations, listFaculty } from '@/lib/vice-principal-faculty-api';
...
export default function VicePrincipalFacultyScreen() {
  ...
  const designationsQuery = useQuery({ queryKey: ['vp-faculty', 'designations'], queryFn: listDesignations });
  const gradesQuery = useQuery({ queryKey: ['vp-faculty', 'grades'], queryFn: listGrades });
  const grade = useMemo(() => gradesQuery.data?.find((g) => g.name === gradeName) ?? null, [gradesQuery.data, gradeName]);

  const sectionsQuery = useQuery({
    queryKey: ['vp-faculty', 'sections', grade?.id],
  ...
  const listQuery = useQuery({
    queryKey: ['vp-faculty', 'list', search, status, designation, grade?.id, section?.id],
    queryFn: () =>
      listFaculty({
              ...
              {faculty.map((member, index) => {
                ...
                return (
                  <Pressable
                    ...
                    onPress={() => router.push(`/(protected)/vice-principal/faculty/${member.id}` as never)}
                  >
                    {member.photoUrl ? (
