...
// requests only, real backend data (GET /staff/me/leave-requests, self-
// scoped -- see vice-principal-my-leave-api.ts's own comment). Distinct
// from any school-wide leave/HR administration, which does not exist in
// this VP mobile app at all.
...
/** The approval_request's own state is authoritative when present (it alone
 * can be CANCELLED, a value the leave row's own state column has no room
 * for) -- falls back to the leave row's own state only if no approval was
 * ever linked. */
...
export default function VicePrincipalMyLeaveScreen() {
  const router = useRouter();
  const query = useQuery({ queryKey: ['vp-my-leave', 'list'], queryFn: listMyLeaveRequests });
  ...
  return (
    <View style={styles.flex}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <Pressable style={styles.applyButton} onPress={() => router.push('/(protected)/vice-principal/my-leave/apply' as never)}>
            ...
            {filtered.map((request, index) => {
              ...
              return (
                <Pressable
                  ...
                  onPress={() => router.push(`/(protected)/vice-principal/my-leave/${request.id}` as never)}
                >
