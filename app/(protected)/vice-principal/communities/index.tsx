// Vice Principal -> Communities (Phase 19) -- school-level oversight of the
// EXISTING Communities/PTA module, real backend data only (communities
// controller, now also authorized for VICE_PRINCIPAL -- see
// vice-principal-communities-api.ts's own comment). Read-only: no create/
...
import { listCommunities } from '@/lib/vice-principal-communities-api';
...
export default function VicePrincipalCommunitiesScreen() {
  const router = useRouter();
  const [academicYearId, setAcademicYearId] = useState<string | null>(null);
  const [state, setState] = useState<string | null>('ACTIVE');

  const yearsQuery = useQuery({ queryKey: ['vp-communities', 'years'], queryFn: listAcademicYears });
  const years = yearsQuery.data ?? [];

  const communitiesQuery = useQuery({
    queryKey: ['vp-communities', 'list', academicYearId, state],
          ...
          <View style={styles.list}>
            {communities.map((community, index) => (
              <Pressable
                ...
                onPress={() => router.push(`/(protected)/vice-principal/communities/${community.id}` as never)}
              >
