// Vice Principal -> Repair & Maintenance request detail (Phase 17) --
// view-only, real backend data only. No assign/start/complete/cancel actions
// -- repair-requests.controller.ts only grants VICE_PRINCIPAL list/overview/
// get; every write method stays ADMIN-only, enforced server-side.
...
// tapping it opens the existing Vice Principal Inventory item-detail screen
// (Phase 15) -- this surfaces the real Inventory -> Maintenance relationship
...
import { getRepairRequest } from '@/lib/vice-principal-maintenance-api';
...
export default function VicePrincipalMaintenanceDetail() {
  const router = useRouter();
  const { id } = useLocalSearchParams<{ id: string }>();

  const requestQuery = useQuery({ queryKey: ['vp-maintenance', 'request', id], queryFn: () => getRepairRequest(id) });
        ...
        {request.inventoryItemId ? (
          <>
            ...
            <Pressable
              ...
              onPress={() => router.push(`/(protected)/vice-principal/inventory/${request.inventoryItemId}` as never)}
            >
