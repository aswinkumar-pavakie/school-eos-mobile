// Vice Principal -> Repair & Maintenance (Phase 17) -- school-level
// operational oversight, real backend data only (repair-requests.controller.ts,
// now also authorized for VICE_PRINCIPAL -- see vice-principal-maintenance-api.ts's
// own comment). Read-only: no create/assign/start/complete/cancel actions
// anywhere in this module. Guarded by the parent vice-principal/_layout.tsx.
...
import { getRepairRequestOverview, listRepairRequests } from '@/lib/vice-principal-maintenance-api';
...
export default function VicePrincipalMaintenanceScreen() {
  ...
  const overviewQuery = useQuery({ queryKey: ['vp-maintenance', 'overview'], queryFn: getRepairRequestOverview });

  const requestsQuery = useQuery({
    queryKey: ['vp-maintenance', 'requests', search, location, status, priority, issueType],
    queryFn: () =>
      listRepairRequests({
            ...
            <View style={styles.list}>
              {requests.map((request, index) => (
                <Pressable
                  ...
                  onPress={() => router.push(`/(protected)/vice-principal/maintenance/${request.id}` as never)}
                >
