...
export default function EventsScreen() {
  ...
  function confirmDelete(event: StudentEvent) {
    Alert.alert('Delete event?', `"${event.name}" and every student's permission request on it will be permanently removed.`, [
  ...
  return (
    <View style={styles.flex}>
      <AppHeader title="Events" subtitle="Create and manage student event permissions" onBack={() => router.replace('/erp')} />
      <ScrollView
      ...
      >
        ...
        {eventsQuery.isLoading ? (
          ...
          (eventsQuery.data ?? []).map((event) => (
            <Pressable key={event.id} style={[styles.card, cardShadow]} onPress={() => router.push(`/events/${event.id}`)}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.cardTeacher} numberOfLines={1}>Monitoring teacher: {event.monitoringTeacherName}</Text>
              </View>
              <Pressable
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', borderRadius: 18, padding: 16, flexDirection: 'row', alignItems: 'flex-start', gap: 10 },
  ...
  cardTeacher: { fontSize: 12.5, color: parentColors.mutedLight, fontFamily: 'PlusJakartaSans_600SemiBold', marginTop: 6 },
