...
// titled sections, 3-column grid, 58px circular blue icon buttons. Only Fees is a
// real, wired destination in this build; every other icon is visual-only (design
// complete, feature not yet built) -- exactly per plan.
...
export default function MyClassScreen() {
  ...
  const subtitle = selected ? [selected.gradeName, selected.sectionName ? `Section ${selected.sectionName}` : null].filter(Boolean).join(' · ') : undefined;
